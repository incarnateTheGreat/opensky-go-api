...
// TestFetchArrivals_MockServer tests the arrivals endpoint with auth
func TestFetchArrivals_MockServer(t *testing.T) {
	mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Verify path
		if r.URL.Path != "/api/flights/arrival" {
			t.Errorf("expected path /api/flights/arrival, got %s", r.URL.Path)
		}

		// Verify query params
		query := r.URL.Query()
		if query.Get("airport") != "KJFK" {
			t.Errorf("expected airport KJFK, got %s", query.Get("airport"))
		}
		...
		// Return mock arrivals
		arrivals := []HistoricalFlight{
			{
				Icao24:              "abc123",
				FirstSeen:           1234500000,
				LastSeen:            1234567890,
				EstArrivalAirport:   strPtr("KJFK"),
				EstDepartureAirport: strPtr("KLAX"),
	...
	// Set up test state
	originalBaseURL := openSkyBaseURL
	originalToken := openSkyAccessToken
	defer func() {
		openSkyBaseURL = originalBaseURL
		openSkyAccessToken = originalToken
	}()

	openSkyBaseURL = mockServer.URL
	openSkyAccessToken = "test-token"
	historicalCache.Clear()

	arrivals, err := fetchArrivals("KJFK", 1234500000, 1234600000)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if arrivals[0].Icao24 != "abc123" {
		t.Errorf("expected icao24 'abc123', got '%s'", arrivals[0].Icao24)
	}
}

// TestFetchDepartures_MockServer tests the departures endpoint
func TestFetchDepartures_MockServer(t *testing.T) {
	mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/flights/departure" {
			t.Errorf("expected path /api/flights/departure, got %s", r.URL.Path)
		}

		departures := []HistoricalFlight{
			{
				Icao24:              "xyz789",
				FirstSeen:           1234500000,
				LastSeen:            1234567890,
	...
	departures, err := fetchDepartures("KLAX", 1234500000, 1234600000)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
// TestFetchArrivals_NoToken tests error when no auth token available
func TestFetchArrivals_NoToken(t *testing.T) {
	originalToken := openSkyAccessToken
	defer func() { openSkyAccessToken = originalToken }()

	openSkyAccessToken = "" // No token
	historicalCache.Clear()

	_, err := fetchArrivals("KJFK", 1234500000, 1234600000)
	if err == nil {
